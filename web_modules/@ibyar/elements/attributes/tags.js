export function getClassRef(htmlElementName) {
    if (Reflect.has(window, htmlElementName)) {
        return Reflect.get(window, htmlElementName);
    }
...
export function findByTagName(tagName) {
...
export function isHTMLUnknownElementTagName(tagName) {
    for (const tag of NativeTags) {
        if (tag.name === tagName) {
            return false;
        }
...
export function getTagName(classRef) {
    for (const tag of NativeTags) {
...
export function isEmptyElement(tagName) {
